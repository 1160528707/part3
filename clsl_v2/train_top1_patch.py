...
"""Training-function patch for the disease-specific CLSL-v2 core.

This file keeps the public `compute_batch_loss_top1` entry point used by
`clsl_v2.train`. It adds support for disease-specific z, conditional evidence
lattice consistency, decoder graph regularization, and transition graph / flow
regularization.
"""
...
def _graph_regularization(model: CLSLv2) -> Dict[str, torch.Tensor]:
    regs: Dict[str, torch.Tensor] = {}
    for name in ["current_decoder", "future_decoder", "transition"]:
        module = getattr(model, name, None)
        if module is not None and hasattr(module, "graph_regularization"):
            for k, v in module.graph_regularization().items():
                regs[f"{name}_{k}"] = v
    return regs
...
def compute_batch_loss_top1(
    ...
    if fine_view in outputs:
        ...
        for v, out in outputs.items():
            if v == fine_view:
                continue
            _, coarse_probs, coarse_entropy = build_loss_and_probs(
                energy_loss, out, "future", batch["y_future"], batch["y_future_mask"]
            )
        ...
        if lattice_terms:
            ...
            loss = loss + float(
                loss_cfg.get("conditional_lattice_consistency", loss_cfg.get("lattice_consistency", 0.0))
            ) * lattice
    ...
    if hasattr(model, "transition") and hasattr(model.transition, "semigroup_loss"):
        current_state = torch.nan_to_num(batch["y_current"].float(), nan=0.0)
        semigroup = model.transition.semigroup_loss(
            primary["z_current"].detach(),
            batch["delta_t"],
            current_state=current_state,
            current_state_mask=batch["y_current_mask"],
        )
        loss = loss + float(loss_cfg.get("transition_semigroup", 0.0)) * semigroup
        stats["transition_semigroup"] = float(semigroup.detach().cpu())

    graph_regs = _graph_regularization(model)
    if graph_regs:
        ...
        for name, value in graph_regs.items():
            ...
            elif name.endswith("graph_residual_sparse"):
                graph_loss = graph_loss + float(loss_cfg.get("graph_residual_sparse", loss_cfg.get("graph_sparse", 0.0))) * value
            ...
            elif name.endswith("transition_graph_sparse"):
                graph_loss = graph_loss + float(loss_cfg.get("transition_graph_sparse", loss_cfg.get("graph_sparse", 0.0))) * value
            elif name.endswith("transition_graph_prior_alignment"):
                graph_loss = graph_loss + float(
                    loss_cfg.get("transition_graph_prior_alignment", loss_cfg.get("graph_prior_alignment", 0.0))
                ) * value
    ...
    tensors = {
        "future_probs": future_probs.detach(),
        "current_probs": current_probs.detach(),
        "future_entropy": future_entropy.detach(),
        "z_current_shape": torch.tensor(primary["z_current"].shape, device=device),
    }

...
class Schema:
    ...
    def view_feature_mask(self, view_name: str) -> List[int]:
        if view_name not in self.views:
            raise KeyError(f"Unknown view '{view_name}'. Available: {list(self.views)}")
        allowed = set(self.views[view_name]["modalities"])
        return [1 if f.modality in allowed else 0 for f in self.feature_defs]
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "Schema":
        return cls(
            diseases=d["diseases"],
            feature_defs=[FeatureDef(**x) for x in d["feature_defs"]],
